...
base = 'ml-100k/u3.base'
test = 'ml-100k/u3.test'
users = pd.read_csv('ml-100k/u.user', sep='|', names=user_cols, encoding='latin-1')
item = pd.read_csv('ml-100k/u.item', sep='|', names=item_cols, encoding='latin-1')
utrain = pd.read_csv(base, sep='\t', names=data_cols, encoding='latin-1')
utest = pd.read_csv(test, sep='\t', names=data_cols, encoding='latin-1')
...
#Vamos recomendar o filme que tiver maior rating medio.
data1 = pd.read_csv(base, sep='\t', names=data_cols, encoding='latin-1')
testdata1 = pd.read_csv(test, sep='\t', names=data_cols, encoding='latin-1')
...
ratings_total1 = pd.DataFrame({'movie id':ratings_total1.index, 'total ratings': ratings_total1.values})
...
#Let's recommend only movies with the best mean rate.
...
for i in recommendation:
    ...
    for j in i:
        r = 0
        for m in final1.index:
            if int(final1['movie id'][m]) == int(j[1]):
                r = final1['rating'][m]
                break
        if(r>0 and r>item[2]):
            item = j
